...
async function faceAllowed(userId, mode) {
  // Billing was removed in the core reset — Face Mode is open to every
  // signed-in user (interview mode stays open to brand-new accounts too).
  if (mode === "interview") return { ok: true };
...
<style>
  ...
  /* TRANSPARENT page: the app's own animated background shows through the
     WebView (which is also set transparent on the Flutter side). Only the
     avatar video itself paints pixels. */
  html,body { margin:0; height:100%; background:transparent; color:var(--mist);
  ...
  /* Strip any solid backgrounds the embed injects around the video. */
  did-agent, [data-name="did-agent"], #did-container { background:transparent !important; }
