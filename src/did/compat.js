...
router.post("/v1/chat/completions", verifyDid, async (req, res) => {
  ...
  try {
    ...
    const extraSystem =
      (userId ? await buildMemoryPrompt(userId) : "") +
      toolCtx.block +
    ...
    // Same quiet learning loop as /chat — face conversations (and the
    // face-to-face first interview especially) teach Hari who this is.
    if (userId && full) {
